...
export default function EmbeddedFinancialServices() {
  return (
    <section id="financial-services" className="py-20 sm:py-28 bg-surface-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid gap-8 md:grid-cols-3">
          {products.map((p) => (
            <div
              key={p.title}
              className="rounded-xl border border-surface-200 bg-white p-8 shadow-sm hover:shadow-md transition text-center"
            >
              <span className="text-3xl" aria-hidden>{p.icon}</span>
              <h3 className="mt-5 text-xl font-semibold text-surface-900">{p.title}</h3>
              <p className="mt-3 text-surface-600">{p.description}</p>
            </div>
          ))}
        </div>
